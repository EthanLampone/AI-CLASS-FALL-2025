    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                write_rewards_to_csv(REWARD_FILE, average_rewards)
                running = False
...
# --- SECTION 4: HEADLESS TRAINING LOGIC (FOR DATA COLLECTION) ---

# New configuration for the plot:
# We'll use a fixed number of episodes (e.g., 500) for consistent comparison.
TRAINING_EPISODES = 500
AVG_WINDOW = 10 # Window size for calculating the moving average reward

def train_headless(decay_value, run_name):
    """
    Runs the full DQN training loop without Pygame, recording episode rewards.
    """
    print(f"--- Starting Headless Training for {run_name} (Decay: {decay_value}) ---")
    ...
    # Reset steps_done for a clean run
    agent.steps_done = 0
    ...
    for episode in range(1, TRAINING_EPISODES + 1):
        ...
        # New loop: Run until episode is done
        while not done:
            # Epsilon calculation now uses the input decay_value
            current_epsilon = EPS_END + (EPS_START - EPS_END) * \
                                math.exp(-1. * agent.steps_done / decay_value)
            ...
            # 3. Store experience and update steps_done
            agent.remember(state, action, reward, next_state, done)
        ...
        if episode % 50 == 0:
            print(f"Episode {episode}/{TRAINING_EPISODES}: Reward = {total_reward:.2f}, Epsilon = {current_epsilon:.4f}")

    print(f"--- Finished Headless Training for {run_name} ---")
    return episode_rewards

# --- SECTION 5: DATA COLLECTION AND PLOTTING ---

# 1. Define the decay values to test
DECAY_RATES = {
    'High Decay (Fast)': 1000,
    'Middle Decay (Balanced)': 5000,
    'Low Decay (Slow)': 20000
}

all_rewards = {}

# 2. Run the headless training for each decay rate
for name, decay in DECAY_RATES.items():
    rewards = train_headless(decay, name)
...
def moving_average(data, window):
    ...
    # Ensure numpy is imported at the top of your script: import numpy as np
    if not data:
        return []
    # Pad the start with the first element to avoid a short initial window
    data = [data[0]] * (window - 1) + data
    # Convolve with a normalized window
    weights = np.repeat(1.0, window) / window
    # Use 'valid' mode to ensure the result is only for full window
    ma = np.convolve(data, weights, 'valid')
    return ma.tolist()

if __name__ == "__main__":

    # RUN THE HEADLESS TRAINING FOR EACH DECAY RATE
    for name, decay in DECAY_RATES.items():
        rewards = train_headless(decay,name)
        all_rewards[name] = rewards
    ...
    for name, rewards in all_rewards.items():
        ma_rewards = moving_average(rewards, AVG_WINDOW)
        plot_data[name] = ma_rewards

    # 5. Plot the Results (Requires matplotlib)
    # Ensure matplotlib is installed: pip install matplotlib
    plt.figure(figsize=(12, 6))
    plt.title('DQN Performance: Average Reward vs. Epsilon Decay Rate')
    plt.xlabel(f'Episode (Moving Average Window: {AVG_WINDOW})')
    plt.ylabel(f'Average Total Reward (over {AVG_WINDOW} Episodes)')

    # Plot each line
    for name, ma_rewards in plot_data.items():
        plt.plot(range(AVG_WINDOW, len(ma_rewards) + AVG_WINDOW), ma_rewards, label=name)

    plt.legend()
    plt.grid(True)
    plt.axhline(y=0, color='r', linestyle='--', linewidth=0.5, alpha=0.5) # Reference line for 0 reward
    plt.show()

    # NOTE: You will need to comment out or remove the 'if __name__ == "__main__": run_visualizer()' 
    # block and replace it with the new data collection and plotting logic to run the comparison.
